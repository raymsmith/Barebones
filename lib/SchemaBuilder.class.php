...
class SchemaBuilder {
	...
	public static function init() {
		// set database
		$db = ApplicationDataConnectionPool::getDefaultConnection();
		if (!$db) {
	 ...
	 * @return Exception $e on error
	 */
	public static function generateSchemas($db_name, $table_name="") {
		// set database name
		SchemaBuilder::$db_name = $db_name;
		$namespace;
			if(preg_match('/^_/',SchemaBuilder::$db_name)){
				$namespace = 'customer';
			}else{
				$namespace = SchemaBuilder::$db_name;
			}
		SchemaBuilder::$namespace = $namespace;
		try {
			...
			if($table_name != "") {
				$sql = "SELECT * FROM `INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA` = '" . SchemaBuilder::$db_name . "' AND `TABLE_NAME` = '" . $table_name . "'";
				$result = SchemaBuilder::$_db->query($sql);
				if($result->num_rows() == 0){
					// throw new \Exception(get_class($this)." Failed to load schema: File does not exist ".SCHEMAPATH.$this->name_space.".".$this->table.".schema.php");
					throw new \Exception("Failed to load schema: Table " . $table_name . " does not exist");
				}

				while($result->hasNext()){
					$row = $result->next();
					SchemaBuilder::buildSchemaFromTable($row['TABLE_NAME']);
				}
			} else {
